...
/* ─── Shared input style ─── */
const inputClass =
  'w-full bg-secondary border border-border px-4 py-2.5 text-[13px] font-light text-foreground placeholder:text-white/25 focus:outline-none focus:border-primary transition-colors';
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="w-full max-w-sm animate-fade-in">
          <h1 className="font-mono text-[12px] tracking-[0.5em] text-primary text-center mb-8">ADMIN</h1>
          <div className="space-y-3">
            ...
            <input
              ...
              className={inputClass}
            />
            <label className="flex items-center gap-2 font-mono text-[10px] text-white/35">
            ...
            <button
              ...
              className="w-full py-2.5 font-mono text-[10px] tracking-[0.25em] uppercase border border-[#C4A35B]/60 text-[#C4A35B] hover:bg-[#C4A35B] hover:text-[#0c0c0e] transition-colors"
            >
              Sign In
            </button>
          </div>
        ...
        <>
          ...
          <aside className="fixed left-0 top-0 bottom-0 w-72 bg-card border-r border-border z-50 overflow-y-auto" style={{ animation: 'slideInLeft 0.2s ease' }}>
            <div className="flex items-center justify-between p-4 border-b border-border">
              <span className="font-mono text-[10px] tracking-[0.35em] uppercase text-primary">ADMIN</span>
            ...
            <nav className="p-4 space-y-1">
              ...
              <a
                href="/"
                className="block px-3 py-2.5 font-mono text-[10px] tracking-[0.2em] text-white/30 hover:text-[#C4A35B] transition-colors mb-3"
              >
                HOME
              </a>
              <div className="border-t border-border pt-3 space-y-0.5">
                {[
                  ...
                  <button
                    ...
                    className={`w-full text-left px-3 py-2.5 font-mono text-[10px] tracking-[0.2em] transition-colors ${tab === index ? 'text-primary' : 'text-white/30 hover:text-[#C4A35B]'}`}
                  >
                    {label}
                  </button>
                ))}
                ...
                <div>
                  <button
                    ...
                    className={`w-full text-left px-3 py-2.5 font-mono text-[10px] tracking-[0.2em] transition-colors ${tab === 3 ? 'text-primary' : 'text-white/30 hover:text-[#C4A35B]'}`}
                  >
                    APPEARANCE
                  </button>
                  <div className="ml-6 mt-0.5 space-y-0.5">
                    {[
                      ...
                      <button
                        ...
                        className={`w-full text-left px-3 py-1.5 font-mono text-[9px] tracking-[0.15em] transition-colors ${
                          tab === 3 && subTab === i ? 'text-primary' : 'text-white/25 hover:text-[#C4A35B]'
                        }`}
                      >
                        {sub}
                      </button>
                ...
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 font-mono text-[10px] tracking-[0.2em] transition-colors ${tab === 4 ? 'text-primary' : 'text-white/30 hover:text-[#C4A35B]'}`}
                >
                  ACCOUNT
                </button>
              </div>
      ...
      <div className="sticky top-0 z-40 bg-background/95 backdrop-blur-sm border-b border-border px-4 py-3 flex items-center justify-between">
        <div className="flex flex-col gap-0.5">
          <div className="flex items-center gap-2">
            ...
            <h1 className="font-mono text-[11px] tracking-[0.35em] text-primary">ADMIN</h1>
          </div>
          {/* Breadcrumb */}
        ...
        <button onClick={app.logoutAdmin} className="font-mono text-[10px] tracking-[0.15em] text-white/30 hover:text-foreground transition-colors flex items-center gap-1.5">
  ...
  const OrderCard = ({ order: o }: { order: typeof orders[0] }) => (
    ...
    >
      ...
      <div className="flex items-start justify-between gap-2">
        <div>
          <p className="font-mono text-[10px] tracking-[0.15em] text-primary">{o.id}</p>
      ...
      <div className="flex justify-between items-center border-t border-border pt-2">
        <p className="font-mono text-[9px] tracking-[0.2em] text-white/30 uppercase">Order Total</p>
        <p className="font-mono text-[14px] text-[#C4A35B]">{o.total.toLocaleString()} THB</p>
      </div>
      ...
      {o.slipUrl && <img src={o.slipUrl} alt="Slip" className="w-full max-w-[120px] border border-border mt-1" />}
    </div>
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-3">
        <p className="font-mono text-[10px] tracking-[0.25em] text-white/30 uppercase">Orders</p>
        <div className="flex items-center gap-2">
...
const SortableImage = ({ id, src, index, isCover, onRemove }: SortableImageProps) => {
  ...
  return (
    <div ref={setNodeRef} style={style} className="relative group w-24 h-32 border border-border overflow-hidden bg-secondary flex-shrink-0">
      <img src={src} alt={`Image ${index + 1}`} className="w-full h-full object-cover" />
      <div {...attributes} {...listeners} className="absolute top-1 left-1 cursor-grab text-foreground/70 hover:text-foreground bg-background/60 p-0.5">
        <GripVertical size={14} />
      </div>
      {isCover && (
        <span className="absolute bottom-1 left-1 font-mono text-[8px] tracking-[0.2em] bg-primary text-primary-foreground px-1.5 py-0.5">
          * Cover
        </span>
      )}
      <button
        type="button"
        onClick={onRemove}
        className="absolute top-1 right-1 bg-background/80 p-0.5 text-white/30 hover:text-destructive transition-colors"
      >
        <X size={12} />
      </button>
    </div>
  ...
  return (
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-3 mb-2">
        <button onClick={onBack} className="font-mono text-[10px] tracking-[0.15em] text-white/30 hover:text-white/60 transition-colors">← Back</button>
        <h2 className="font-mono text-[10px] tracking-[0.35em] uppercase text-foreground">BULK UPLOAD</h2>
      </div>

      {/* ── STEP 1: Upload Images ── */}
      <div className="border border-border p-5 space-y-4">
        <div className="flex items-center gap-2">
          <span className="font-mono text-[10px] bg-primary text-primary-foreground w-5 h-5 flex items-center justify-center">1</span>
          <p className="font-mono text-[10px] tracking-[0.2em] uppercase text-foreground">UPLOAD IMAGE FOLDERS</p>
        ...
        <input
          ...
          className="w-full font-mono text-[10px] text-white/30 bg-background border border-border px-3 py-2 file:mr-3 file:text-[10px] file:border-0 file:bg-primary file:text-primary-foreground file:px-3 file:py-1 file:cursor-pointer"
        />
          ...
          <div className="space-y-2">
            ...
            {!uploadingImages && (
              <button
                ...
                className="border border-[#C4A35B]/60 text-[#C4A35B] px-5 py-2 font-mono text-[9px] tracking-[0.2em] uppercase hover:bg-[#C4A35B] hover:text-[#0c0c0e] transition-colors"
              >
      ...
      <div className="border border-border p-5 space-y-4">
        <div className="flex items-center gap-2">
          <span className="font-mono text-[10px] bg-primary text-primary-foreground w-5 h-5 flex items-center justify-center">2</span>
          <p className="font-mono text-[10px] tracking-[0.2em] uppercase text-foreground">UPLOAD CSV</p>
        ...
        <div
          ...
          className={`border-2 border-dashed p-6 text-center transition-colors ${dragOverCSV ? 'border-primary bg-primary/5' : 'border-border'}`}
        >
          <p className="text-xs text-muted-foreground mb-3">Drag & drop CSV หรือ</p>
          <label className="cursor-pointer border border-border text-white/30 hover:border-primary hover:text-primary transition-colors px-4 py-2 font-mono text-[9px] tracking-[0.15em] uppercase">
            เลือกไฟล์ CSV
            ...
            <table className="w-full text-xs">
              <thead>
                <tr className="border-b border-border bg-background">
                  <th className="px-3 py-2 text-left font-mono text-[9px] tracking-[0.2em] uppercase text-white/30">Status</th>
                  <th className="px-3 py-2 text-left font-mono text-[9px] tracking-[0.2em] uppercase text-white/30">ID</th>
                  <th className="px-3 py-2 text-left font-mono text-[9px] tracking-[0.2em] uppercase text-white/30">Title</th>
                  <th className="px-3 py-2 text-left font-mono text-[9px] tracking-[0.2em] uppercase text-white/30">Platform</th>
                  <th className="px-3 py-2 text-left font-mono text-[9px] tracking-[0.2em] uppercase text-white/30">Price</th>
                  <th className="px-3 py-2 text-left font-mono text-[9px] tracking-[0.2em] uppercase text-white/30">Front</th>
                  <th className="px-3 py-2 text-left font-mono text-[9px] tracking-[0.2em] uppercase text-white/30">Back</th>
                </tr>
              </thead>
              <tbody>
                {rows.map((row, i) => {
                    ...
                    <tr key={i} className="border-b border-border/50 hover:bg-background/50">
                      <td className="px-3 py-2">
                        <span className={`font-mono text-[9px] px-2 py-0.5 border ${
                          row.status === 'done' ? 'text-green-400 border-green-500/30' :
                          row.status === 'error' ? 'text-red-400 border-red-500/30' :
                          row.status === 'uploading' ? 'text-primary border-primary/30' :
                      ...
                      <td className="px-3 py-2">
                        {isBase64Front
                          ? <img src={row.imageFrontFilename} className="w-8 h-10 object-cover border border-border" />
                      ...
                      <td className="px-3 py-2">
                        {isBase64Back
                          ? <img src={row.imageBackFilename} className="w-8 h-10 object-cover border border-border" />
      ...
      {rows.length > 0 && (
        <div className="flex flex-col gap-3">
          ...
          <button
            ...
            disabled={uploading}
            className="border border-[#C4A35B]/60 text-[#C4A35B] px-6 py-2.5 font-mono text-[10px] tracking-[0.2em] uppercase hover:bg-[#C4A35B] hover:text-[#0c0c0e] transition-colors disabled:opacity-50"
          >
...
const ProductsTab = () => {
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
        <div className="flex justify-between items-center">
          <h2 className="font-mono text-[10px] tracking-[0.3em] uppercase text-foreground">{editing === 'new' ? 'ADD PRODUCT' : 'EDIT PRODUCT'}</h2>
          ...
          <DndContext collisionDetection={closestCenter} onDragEnd={handleImageDragEnd}>
            <SortableContext items={imageList.map((i) => i.id)} strategy={horizontalListSortingStrategy}>
              <div className="flex gap-3 flex-wrap">
                ...
                {/* Add button */}
                <label className="w-24 h-32 border border-dashed border-border bg-secondary/50 flex flex-col items-center justify-center cursor-pointer hover:border-primary transition-colors flex-shrink-0">
        ...
        <button onClick={handleSave} className="w-full py-2.5 font-mono text-[10px] tracking-[0.25em] uppercase border border-[#C4A35B]/60 text-[#C4A35B] hover:bg-[#C4A35B] hover:text-[#0c0c0e] transition-colors flex items-center justify-center gap-2">
  ...
  return (
    <div className="space-y-3">
            ...
            <>
              ...
              <div className="absolute right-0 top-full mt-1.5 z-20 bg-card border border-border shadow-lg overflow-hidden min-w-[140px]">
                ...
                >
                  ...
                  <div>
                    <p className="font-mono text-[9px] tracking-[0.2em] uppercase">SINGLE</p>
                ...
                >
                  ...
                  <div>
                    <p className="font-mono text-[9px] tracking-[0.2em] uppercase">BULK</p>
      ...
      {filteredByStock.length === 0 && <p className="font-mono text-[10px] tracking-[0.2em] uppercase text-white/20 mt-2">No products found.</p>}
        ...
        >
          ...
          {p.frontImage ? (
            <img
              ...
              className={`w-10 h-14 object-cover flex-shrink-0 ${!selectMode ? 'cursor-pointer hover:ring-1 hover:ring-primary transition-all' : ''}`}
            />
          ) : (
            <div className="w-10 h-14 bg-secondary border border-border flex items-center justify-center flex-shrink-0"><ImageIcon size={14} className="text-muted-foreground" /></div>
          )}
          <div className="flex-1 min-w-0">
            <p className="text-[13px] font-light text-foreground truncate">{p.title}</p>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-2">
          {filteredByStock.map((p, index) => (
            ...
            >
              ...
              <div className="p-2">
                <p className="font-mono text-[10px] text-foreground leading-snug line-clamp-2">{p.title}</p>
                <p className="text-[10px] text-muted-foreground">{p.id}</p>
                <div className="mt-1 flex items-center justify-between">
                  {discountRate > 0 ? (
                    <div className="flex flex-col">
                      <span className="font-mono text-[11px] text-[#C4A35B]">{Math.round(p.price * (1 - discountRate / 100)).toLocaleString()} THB</span>
                      <span className="text-[10px] text-muted-foreground line-through">{p.price.toLocaleString()}</span>
                    </div>
                  ) : (
                    <span className="font-mono text-[11px] text-[#C4A35B]">{p.price.toLocaleString()} THB</span>
                  )}
                  {!selectMode && (
    ...
    <div className="max-w-md space-y-5">
      ...
      <div>
        ...
        <div className="flex items-start gap-4 mb-3">
          {localLogoUrl ? (
            <div className="relative group">
              <img
                ...
                className="border border-border object-contain bg-secondary/50"
              />
              <button
                type="button"
                onClick={() => setLocalLogoUrl('')}
                className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-background border border-border text-white/30 hover:text-destructive flex items-center justify-center transition-colors"
              >
                <X size={10} />
              </button>
            </div>
          ) : (
            <label className="flex flex-col items-center justify-center w-20 h-20 border border-dashed border-border bg-secondary/50 cursor-pointer hover:border-primary transition-colors">
              <Upload size={18} className="text-muted-foreground mb-1" />
          ...
          {localLogoUrl && (
            <label className="flex flex-col items-center justify-center w-10 h-10 mt-1 border border-dashed border-border bg-secondary/50 cursor-pointer hover:border-primary transition-colors">
      ...
      <div className="border border-border p-5 bg-card space-y-3">
        ...
        <div className="bg-background/50 border border-border px-4 py-2 flex justify-center">
      ...
      <button onClick={handleSave} className="border border-[#C4A35B]/60 text-[#C4A35B] px-6 py-2 font-mono text-[9px] tracking-[0.2em] uppercase hover:bg-[#C4A35B] hover:text-[#0c0c0e] transition-colors">Save</button>
    </div>
  );
};
  ...
  return (
    <div className="relative flex-1">
      ...
      {open && filtered.length > 0 && (
        <div className="absolute z-50 top-full mt-1 left-0 right-0 bg-card border border-border shadow-lg overflow-hidden">
          {filtered.map((s, i) => (
    ...
    <div className="relative" style={{ minHeight: '70vh' }}>
      ...
      <div className="flex items-center gap-2 mb-4 flex-wrap">
        ...
        <div className="flex gap-1 border border-border p-0.5">
          {(['platform', 'genre'] as const).map(t => (
            <button key={t} onClick={() => { setActiveTab(t); setSelectedCat(''); setSearch(''); }}
              className={`px-4 py-1 font-mono text-[10px] tracking-[0.18em] transition-colors ${activeTab === t ? 'bg-primary text-primary-foreground' : 'text-white/30 hover:text-white/60'}`}>
        ...
        <div className="relative flex-1 max-w-sm">
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by title, platform, genre..."
            className="w-full bg-secondary border border-border px-3 py-1.5 font-mono text-[10px] text-foreground placeholder:text-white/25 focus:outline-none focus:border-primary" />
        ...
        {/* Active filter tag */}
        {selectedCat && (
          <span className="inline-flex items-center gap-1 border border-primary/40 text-primary font-mono text-[9px] px-2.5 py-0.5">
            {selectedCat}
        ...
        <button onClick={() => setDrawerOpen(true)}
          className={`flex items-center gap-1.5 px-3 py-1.5 border font-mono text-[9px] tracking-[0.18em] uppercase transition-colors bg-background/80 backdrop-blur-sm ${selectedCat ? 'border-primary text-primary' : 'border-border text-white/30 hover:text-primary hover:border-primary'}`}>
          <SlidersHorizontal size={12} />
          FILTER
          {selectedCat && <span className="w-4 h-4 bg-primary text-primary-foreground font-mono text-[9px] flex items-center justify-center">1</span>}
        </button>
      </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
          {filteredProducts.map(p => (
            <button key={p.id} onClick={() => { setModalProduct(p); setModalVal(activeTab === 'platform' ? p.platform : p.genre); }}
              className="group border border-border/50 overflow-hidden text-left hover:border-primary transition-colors cursor-pointer">
          ...
          <div ref={drawerRef} className="fixed right-0 top-0 bottom-0 w-72 bg-card border-l border-border z-50 overflow-y-auto flex flex-col"
            ...
            <div className="flex-1 px-4 py-3 space-y-0.5 overflow-y-auto">
              ...
              <button onClick={() => { setSelectedCat(''); setDrawerOpen(false); }}
                className={`w-full text-left px-2 py-2 font-mono text-[10px] tracking-[0.12em] transition-colors ${!selectedCat ? 'text-primary' : 'text-muted-foreground hover:text-foreground'}`}>
              ...
              {sidebarItems.map(val => {
                ...
                return (
                  <div key={val} className="group/row">
                    {renamingCat === val ? (
                      <div className="px-2 py-1.5 space-y-2">
                        <AutocompleteInput value={renameVal} onChange={setRenameVal}
                          ...
                          className="w-full bg-background border border-primary px-2.5 py-1.5 font-mono text-[10px] text-foreground focus:outline-none"
                        />
                        <div className="flex gap-2">
                          <button onClick={handleRename} disabled={renameSaving}
                            className="flex-1 flex items-center justify-center gap-1 py-1.5 bg-primary text-primary-foreground font-mono text-[9px] tracking-[0.15em] hover:opacity-90 disabled:opacity-50">
                            {renameSaving ? <Loader2 size={12} className="animate-spin" /> : <Save size={12} />}
                            บันทึก
                          </button>
                          <button onClick={() => setRenamingCat('')}
                            className="px-3 py-1.5 border border-border font-mono text-[9px] text-white/30 hover:text-foreground">
                            ยกเลิก
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center transition-colors hover:bg-secondary/30">
                        ...
                        <div className="flex items-center gap-0.5 pr-2 opacity-0 group-hover/row:opacity-100 transition-opacity">
                          <button onClick={() => { setRenamingCat(val); setRenameVal(val); }}
                            className="p-1 text-muted-foreground hover:text-primary"><Edit2 size={11} /></button>
      ...
      {modalProduct && (
        <div className="fixed inset-0 bg-background/80 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-card border border-border w-full max-w-sm shadow-xl">
            <div className="flex items-center justify-between px-5 py-4 border-b border-border">
            ...
            <div className="flex items-center gap-3 px-5 py-4 border-b border-border">
              {modalProduct.frontImage && <img src={modalProduct.frontImage} className="w-12 h-16 object-cover flex-shrink-0" />}
            ...
            <div className="px-5 py-4 space-y-2">
              ...
              <AutocompleteInput value={modalVal} onChange={setModalVal}
                ...
                suggestions={suggestions}
                className="w-full bg-background border border-border px-3 py-2.5 text-[13px] font-light text-foreground focus:outline-none focus:border-primary transition-colors"
              />
            ...
            <div className="flex gap-2 px-5 pb-5">
              <button onClick={handleModalSave} disabled={modalSaving || !modalVal.trim()}
                className="flex-1 flex items-center justify-center gap-2 py-2.5 bg-primary text-primary-foreground font-mono text-[10px] tracking-[0.2em] hover:opacity-90 disabled:opacity-50 transition-opacity">
                {modalSaving ? <Loader2 size={14} className="animate-spin" /> : <Save size={14} />}
                บันทึก
              </button>
              <button onClick={() => setModalProduct(null)}
                className="px-4 py-2.5 border border-border font-mono text-[10px] text-white/30 hover:text-foreground transition-colors">
                ยกเลิก
              </button>
            </div>
          </div>
    ...
    <div className="max-w-md space-y-5">
      ...
      <div>
        ...
        <div className="flex items-start gap-3">
          {qr && (
            <div className="relative group">
              <img src={qr} alt="QR" className="w-24 h-24 border border-border object-contain bg-secondary" />
              <button
                type="button"
                onClick={() => setQr('')}
                className="absolute -top-1.5 -right-1.5 w-5 h-5 bg-background border border-border text-white/30 hover:text-destructive flex items-center justify-center transition-colors"
          ...
          <label className="flex flex-col items-center justify-center w-24 h-24 border border-dashed border-border bg-secondary/50 cursor-pointer hover:border-primary transition-colors">
            <Upload size={18} className="text-muted-foreground mb-1" />
      ...
      <button onClick={() => { app.setBankInfo(bank); app.setQrCodeUrl(qr); app.setCustomerNote(note); app.setShippingFee(shippingFee); toast.success('Payment details updated'); }} className="border border-[#C4A35B]/60 text-[#C4A35B] px-6 py-2 font-mono text-[9px] tracking-[0.2em] uppercase hover:bg-[#C4A35B] hover:text-[#0c0c0e] transition-colors">Save</button>
    </div>
  );
};
    ...
    <div className="max-w-md space-y-5">
      ...
      <div className="border border-border p-5 bg-card space-y-3">
        ...
        <div className="border-t border-dashed border-border pt-3 space-y-2">
          <button className="w-full border border-border text-white/30 py-2 font-mono text-[10px] tracking-[0.2em]">
            {cfg.copyButtonText}
          </button>
          <button className="w-full border border-border text-white/30 py-2 font-mono text-[10px] tracking-[0.2em]">
      ...
      <button onClick={() => { setThankYouConfig(cfg); toast.success('Order confirmation updated'); }} className="border border-[#C4A35B]/60 text-[#C4A35B] px-6 py-2 font-mono text-[9px] tracking-[0.2em] uppercase hover:bg-[#C4A35B] hover:text-[#0c0c0e] transition-colors">Save</button>
    </div>
  );
};
...
const ConditionSettingsSubTab = () => {
  ...
  return (
    <div className="max-w-md space-y-4">
      ...
      {conditionGrades.map((g) => (
        <div key={g.id} className="flex items-start gap-3 p-3 bg-card border border-border">
          <div className="w-8 h-8 border border-primary flex items-center justify-center font-mono text-[11px] text-primary flex-shrink-0">{g.code}</div>
          <div className="flex-1 min-w-0">
      ...
      {editId && (
        <div className="space-y-3 p-4 border border-primary/30 bg-card animate-fade-in">
          ...
          <div className="flex gap-2">
            <button onClick={handleSave} className="border border-[#C4A35B]/60 text-[#C4A35B] px-4 py-1.5 font-mono text-[9px] tracking-[0.2em] uppercase hover:bg-[#C4A35B] hover:text-[#0c0c0e] transition-colors">Save</button>
...
const AccountTab = () => {
  ...
  return (
    <div className="max-w-md space-y-4">
      ...
      <button onClick={() => { if (password) { setAdminCredentials(email, password); toast.success('Credentials updated'); } else { toast.error('Enter a new password'); } }} className="border border-[#C4A35B]/60 text-[#C4A35B] px-6 py-2 font-mono text-[9px] tracking-[0.2em] uppercase hover:bg-[#C4A35B] hover:text-[#0c0c0e] transition-colors">Update</button>
    </div>
  );
};
